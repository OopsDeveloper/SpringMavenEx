...
public class PartitionByEX {
    public static void main(String[] args) {
        Student2[] stuArr = {
                ...
                new Student2("김자미",false,1,1,250),
        ...
        Map<Boolean, Map<Boolean, List<Student2>>> faildStuBySex = Stream.of(stuArr)
                .collect(partitioningBy(Student2::isMale, partitioningBy(s -> s.getScore() <= 100)));
        List<Student2> faildMaleStu = faildStuBySex.get(true).get(true);
        List<Student2> faildFemaleStu = faildStuBySex.get(false).get(true);

        for (Student2 s : faildMaleStu) {
            System.out.println("s = " + s);
        }
        for (Student2 s : faildFemaleStu) {
            System.out.println("s = " + s);
        }
    }
}
